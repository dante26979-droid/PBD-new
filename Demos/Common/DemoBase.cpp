...
#include "Simulation/DebugOptions.h"
...
namespace
{
	bool parsePositiveUnsignedArgument(const char* value, unsigned int& out)
	{
		char* end = nullptr;
...
void DemoBase::init(int argc, char** argv, const char* demoName)
{
	...
	m_sceneFile = "";
	setUseCache(true);
	const DemoGuiStartupPolicy guiStartupPolicy = parseDemoGuiStartupPolicy(argc, argv);
	for (int i = 1; i < argc; i++)
	{
		string argStr = argv[i];
		if (argStr == "--no-cache")
			setUseCache(false);
		else if (argStr == "--steps-per-render")
		{
			if (i + 1 < argc)
			{
				unsigned int stepsPerRender = 0u;
				if (parsePositiveUnsignedArgument(argv[++i], stepsPerRender))
					m_numberOfStepsPerRenderUpdate = stepsPerRender;
				else
					LOG_ERR << "Invalid --steps-per-render value; using current default.";
			}
		}
		else if (isDemoBaseArgumentWithValue(argStr))
		{
			if (i + 1 < argc)
				i++;
		}
		else if (isDemoBaseFlagArgument(argStr))
			continue;
		else
		{
	...
	m_gui->init();
	m_gui->setPanelVisible(guiStartupPolicy.showPanel);

	// OpenGL
	MiniGL::init(argc, argv, 1800, 1300, demoName, m_gui->getVSync(), m_gui->getMaximized());
	MiniGL::initLights();
	//MiniGL::initTexture();
	MiniGL::initTexture4pic();	//脊椎场景
	...
	MiniGL::setHapticSelectionFunc(hapticSelection, this);
...
void DemoBase::render()
{
	...
	// Render surgical tool
	//const float surgToolColor[4] = { 0.5, 0.5, 0.5 , 1.0 };
	//MiniGL::drawSurgTool(m_surgToolMeshPairs[0].surgToolVDs[0].getVertices(),
	//	m_surgToolMeshPairs[0].surgToolMeshs[0].getFaces(), m_surgToolMeshPairs[0].surgToolMeshs[0].getFaceNormals(), surgToolColor);
	...
	// Render non textured rigid body
	...
	// render Textured rigid body
	float whiteColor[4] = { 1.0f, 1.0f, 1.0f, 1 };
	for (size_t i = 0; i < rb.size(); i++)
	{
		const VertexData& vd = rb[i]->getGeometry().getVertexData();
		const IndexedFaceMesh& mesh = rb[i]->getGeometry().getMesh();
		...
		if (mesh.getUVs().size() > 0)
		{
			shaderTexBegin(whiteColor);

			Visualization::drawTexturedMesh(vd, mesh, 0, whiteColor);
...
void DemoBase::Myrender()
{
	...
	// Render surgical tool
	const float surgToolColor[4] = { 1,1, 1, 1.0f };
	MiniGL::drawSurgTool(m_surgToolMeshPairs[0].surgToolVDs[0].getVertices(),m_surgToolMeshPairs[0].surgToolMeshs[0].getFaces(), m_surgToolMeshPairs[0].surgToolMeshs[0].getFaceNormals(), surgToolColor);

	MiniGL::drawMesh(m_surgToolMeshPairs[0].surgToolVDs[0].getVertices(), m_surgToolMeshPairs[0].surgToolMeshs[0].getFaces(), m_surgToolMeshPairs[0].surgToolMeshs[0].getFaceNormals(), surgToolColor);
	// Draw sim model	
	SimulationModel* model = Simulation::getCurrent()->getModel();
	if (model == nullptr)
	{
	...
	for (size_t i = 0; i < rb.size(); i++)
	{
		...
		if (mesh.getUVs().size() > 0)
		{
			...
			Visualization::drawTexturedMesh(vd, mesh, 0, whiteColor, texNum);
			texNum++;
	...
	renderTriangleModels();
	renderTetModels();
	float Cons[4] = { 0.0f, 0.4f, 0.0f, 1.0f };
	for (size_t i = 0; i < constraints.size(); i++)
	{
		if (constraints[i] == NULL)continue;
		//if (constraints[i]->getTypeId() == DistanceConstraint_XPBD::TYPE_ID) {
		//	MiniGL::drawVector(pd.getPosition(constraints[i]->m_bodies[0]), pd.getPosition(constraints[i]->m_bodies[1]), 0.2, Cons);
		//}
		if (constraints[i]->getTypeId() == BallJoint::TYPE_ID)
		{
			renderBallJoint(*(BallJoint*)constraints[i]);
	...
	//	MiniGL::drawSphere(pd.getPosition(m_selectedParticles[j]), 0.08f, red);
	//}
	for (unsigned int m = 0; m < m_selectedParticlesRange.size(); m++)
		MiniGL::drawSphere(pd.getPosition(m_selectedParticlesRange[m]), 0.08f, red);
	m_gui->update();
	//for (unsigned int pid : cutPoints) {
	//	if (pid > pd.size()) {
	//		printf("粒子数组越界\n");
	//		continue;
	//	}
	//	MiniGL::drawSphere(pd.getPosition(pid), 0.08f, red);
	//}
	//for (unsigned int pid : keepPoints) {
	//	if (pid > pd.size()) {
	//		printf("粒子数组越界\n");
	//		continue;
	//	}
	//	MiniGL::drawSphere(pd.getPosition(pid), 0.08f,Yellow );
	//}

	const Vector3r& hPos = MiniGL::getHapticPos();
	//MiniGL::drawSphere(hPos, 0.08f, red);
}

void DemoBase::loadSurgToolMeshPair(const std::string& filename0, const std::string& filename1, const Vector3r& translation, const Matrix3r& rotation, const Vector3r& scale)
{
	if (m_totalSurgToolNbr < m_MaxSurgToolNbr && (filename0 != ""))
	{
		string extfileName = FileSystem::normalizePath(getExePath() + filename0);
		loadMesh(extfileName, m_surgToolMeshPairs[m_totalSurgToolNbr].surgToolVDs[SurgToolMeshPair::STS_INACTIVE],
			m_surgToolMeshPairs[m_totalSurgToolNbr].surgToolMeshs[SurgToolMeshPair::STS_INACTIVE],
			translation, rotation, scale);

		m_surgToolMeshPairs[m_totalSurgToolNbr].hasActiveState = false;
		if (filename1 != "")
		{
			extfileName = FileSystem::normalizePath(getExePath() + filename1);
			loadMesh(extfileName, m_surgToolMeshPairs[m_totalSurgToolNbr].surgToolVDs[SurgToolMeshPair::STS_ACTIVE],
				m_surgToolMeshPairs[m_totalSurgToolNbr].surgToolMeshs[SurgToolMeshPair::STS_ACTIVE],
				translation, rotation, scale);
			m_surgToolMeshPairs[m_totalSurgToolNbr].hasActiveState = true;
		}
...
void DemoBase::renderTetModels()
{
	...
	const ParticleData& pd = model->getParticles();
	float surfaceColor[4] = { 0.1f, 0.4f, 0.7f, 1 };
	float fracturePatchColor[4] = { 1.0f, 0.38f, 0.05f, 1.0f };
	float fractureDebugSurfaceColor[4] = { 0.6f, 0.6f, 0.6f, 1.0f };
	const bool fractureColorCode = DebugOptions::fractureDisplayColorCodeEnabled();
	...
	for (unsigned int i = 0; i < model->getTetModels().size(); i++)
	{
		...
		if (vdVis.size() > 0)
		{
			const VertexData& renderVD = tetModel->getRenderVisVertices().size() > 0u ?
				tetModel->getRenderVisVertices() :
				vdVis;
			const IndexedFaceMesh& visMesh = tetModel->getRenderVisMesh().numVertices() > 0u ?
				tetModel->getRenderVisMesh() :
				tetModel->getVisMesh();
			const float* drawColor = fractureColorCode ? fractureDebugSurfaceColor : surfaceColor;
			if (fractureColorCode)
				glUniform3fv(m_shader.getUniform("surface_color"), 1, drawColor);
			Visualization::drawMesh(renderVD, visMesh, 0, drawColor);
		}
		else
		{
			const IndexedFaceMesh& surfaceMesh = tetModel->getRenderVisMesh().numVertices() > 0u ?
				tetModel->getRenderVisMesh() :
				tetModel->getSurfaceMesh();
			const unsigned int offset = tetModel->getIndexOffset();
			const float* drawColor = fractureColorCode ? fractureDebugSurfaceColor : surfaceColor;
			if (fractureColorCode)
				glUniform3fv(m_shader.getUniform("surface_color"), 1, drawColor);
			Visualization::drawMesh(pd, surfaceMesh, offset, drawColor);
		}
		const std::vector<TetModel::FracturePatchDebugInfo>& debugInfo = tetModel->getFracturePatchDebugInfo();
		...
		for (size_t patchIndex = 0u; patchIndex < patches.size(); patchIndex++)
		{
			...
			{
				float patchColor[4] = { fracturePatchColor[0], fracturePatchColor[1], fracturePatchColor[2], fracturePatchColor[3] };
				if (fractureColorCode)
				{
					const TetModel::FracturePatchDebugType type =
						patchIndex < debugInfo.size() ?
						debugInfo[patchIndex].type :
						TetModel::FracturePatchDebugType::Unknown;
					if (patchIndex >= debugInfo.size())
						mismatchCount++;
					TetModel::colorForFracturePatchDebugType(type, patchColor);
				}
				glUniform3fv(m_shader.getUniform("surface_color"), 1, patchColor);
				Visualization::drawMesh(patch.getRenderVertexData(), patch.getRenderSurfaceMesh(), 0, patchColor);
			}
		}
		if (fractureColorCode && (mismatchCount > 0u))
		{
			LOG_WARN << "[FRACTURE_COLOR] mismatch: patches=" << patches.size()
				<< " debugInfo=" << debugInfo.size();
		}
	}
...
void DemoBase::hapticSelection(const Vector3r& pt, void* clientData)
{
	SimulationModel* model = Simulation::getCurrent()->getModel();
	DemoBase* base = (DemoBase*)clientData;

	//std::vector<unsigned int> hits;
	base->m_prevSelectedParticles = base->m_selectedParticles;
	if (!base->m_multiSelection) {
		base->m_selectedParticles.clear();
		base->m_selectedParticlesRange.clear();
	}
	ParticleData& pd = model->getParticles();
	if (pd.size() > 0)
	{
		ToolTargetInteractionController controller;
		ToolProfile profile;
		controller.rebuildParticleIndex(pd.getVertices(), profile.particleSelectRadius);
		unsigned int hit = 0u;
		Real distance = REAL_MAX;
		if (controller.queryNearestParticle(pt, profile.particleSelectRadius, hit, distance))
		{
			base->m_selectedParticles.push_back(hit);
		}
	}
}
	...
	const Matrix3r& rotation, const Vector3r& scale)
{
	...
	if (ext == "OBJ")
	{
		...
		vd.reserve(nPoints);
		bool* pTexReady = new bool[nPoints];
		...
		std::vector<Vector2r> tmpUVs;
		tmpUVs.resize(nPoints);
		for (unsigned int i = 0; i < nFaces; i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				...
				if (nTexCoords > 0)
				{
					if (!pTexReady[posIndices[j]])
					{
						texIndices[j] = faces[i].texIndices[j];

						tmpUVs[posIndices[j]] = Vector2r(texCoords[texIndices[j]][0], texCoords[texIndices[j]][1]);
						mesh.addUVIndex(posIndices[j]);
						pTexReady[posIndices[j]] = true;
					}
				}
			}
		...
		if (nTexCoords > 0) {
			for (unsigned int i = 0; i < nPoints; i++)
			{
				mesh.addUV(tmpUVs[i][0], tmpUVs[i][1]);
			}
		}
